(function () {
    // ---------------------------------------------------------------
    // Site-wide fix: native form controls (file picker text, browser
    // autofill) ignore inherited CSS color and default to black text,
    // which disappears against dark-mode backgrounds. Inject one
    // shared stylesheet so every event page (all themes) stays legible.
    // ---------------------------------------------------------------
    if (!document.getElementById('payment-upload-visibility-fix')) {
        const style = document.createElement('style');
        style.id = 'payment-upload-visibility-fix';
        style.textContent = `
    ...
    forms.forEach((form) => {
        ...
        // Reset any stale upload reference from a previous session.
        form.dataset.paymentScreenshotUrl = '';
        form.dataset.paymentScreenshotId = '';
        ...
        container.innerHTML = `
           ...
           >
            <label class="text-xs font-bold uppercase tracking-wider" style="color: ${accent}">
                ...
                <input type="file" accept="image/jpeg,image/png,image/webp" required class="mt-2 block w-full rounded-lg border border-current bg-transparent px-3 py-2 text-xs" style="color: inherit;">
            </label>
       ...
       uploadButton.addEventListener('click', async () => {
            ...
            try {
                ...
                // Make the uploaded screenshot retrievable by the page's own
                // registration submit handler, so it gets saved alongside
                // the rest of the entry instead of living only in Drive.
                const driveFileId = result.drive_file_id || '';
                form.dataset.paymentScreenshotId = driveFileId;
                form.dataset.paymentScreenshotUrl = driveFileId
